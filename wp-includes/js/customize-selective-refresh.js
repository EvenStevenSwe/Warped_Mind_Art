...
wp.customize.selectiveRefresh = ( function( $, api ) {
	...
	Partial = self.Partial = api.Class.extend(/** @lends wp.customize.SelectiveRefresh.Partial.prototype */{
		...
		renderContent: function( placement ) {
			...
			// Currently a subclass needs to override renderContent to handle _partials returning data object.
			...
			// Prevent placement container from being re-triggered as being rendered among nested _partials.
	...
	self.Placement = Placement = api.Class.extend(/** @lends wp.customize.selectiveRefresh.prototype */{
		 ...
		 * This container element itself will be replaced for _partials that
	...
	/**
	 * Currently-requested _partials and their associated deferreds.
	 *
	 * @since 4.5.0
	...
	/**
	 * Current jqXHR for the request to the _partials.
	 *
	 * @since 4.5.0
			...
			function() {
				...
				_.each( self._pendingPartialRequests, function( pending, partialId ) {
					...
					if ( ! self.partial.has( partialId ) ) {
						 ...
						 * is the context information that may be needed to render certain _partials, such as
				...
				request.done( function( data ) {
					...
					/**
					 * Announce the data returned from a request to render _partials.
					 *
					 ...
					 * injected into the page to support the rendered _partials.
					 * This is similar to the 'saved' event.
					 */
					self.trigger( 'render-_partials-response', data );
	...
	/**
	 * Add _partials for any nav menu container elements in the document.
	 *
	...
	self.addPartials = function( rootElement, options ) {
		...
		containerElements.each( function() {
			...
			/*
			 * Only trigger renders on (nested) _partials that have been not been
	...
	api.bind( 'preview-ready', function() {
		...
		_.each( self.data.partials, function( data, id ) {
			...
			if ( ! partial ) {
				...
				partial = new Constructor(
					id,
					_.extend( { params: data }, data ) // Inclusion of params alias is for back-compat for custom _partials that expect to augment this property.
				);
				self.partial.add( partial );
			} else {
		...
		// Add (dynamic) initial _partials that are declared via data-* attributes.
		self.addPartials( document.documentElement, {
			triggerRendered: false
		} );

		// Add new dynamic _partials when the document changes.
		 ...
		 */
		api.selectiveRefresh.bind( 'render-_partials-response', function handleSettingValiditiesResponse( data ) {
		...
		api.preview.bind( 'active', function() {
			...
			// Make all _partials ready.
			self.partial.each( function( partial ) {
				partial.deferred.ready.resolve();
			} );

			// Make all _partials added henceforth as ready upon add.
