...
wp.customize.navMenusPreview = wp.customize.MenusCustomizerPreview = ( function( $, _, wp, api ) {
	...
	self.init = function() {
		...
		if ( api.selectiveRefresh ) {
			...
			/*
			 * Ensure that wp_nav_menu() instances nested inside of other _partials
			 * will be recognized as being present on the page.
			 */
			api.selectiveRefresh.bind( 'render-_partials-response', function( response ) {
	...
	if ( api.selectiveRefresh ) {
		...
		/**
		 * Request full refresh if there are nav menu instances that lack _partials which also match the supplied args.
		 *
		...
		/**
		 * Handle change for nav_menu[] setting for nav menu instances lacking _partials.
		 *
		 * @since 4.5.0
		 *
		...
		/**
		 * Handle change for nav_menu_item[] setting for nav menu instances lacking _partials.
		 *
		 * @since 4.5.0
		 *
		...
		/**
		 * Handle change for nav_menu_locations[] setting for nav menu instances lacking _partials.
		 *
		 * @since 4.5.0
		 *
		 * @this {wp.customize.Value}
