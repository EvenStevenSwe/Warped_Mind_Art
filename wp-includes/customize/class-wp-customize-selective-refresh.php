...
final class WP_Customize_Selective_Refresh {
	...
	/**
	 * Query var used in requests to render _partials.
	 *
	 * @since 4.5.0
	 */
	...
	/**
	 * Log of errors triggered when _partials are rendered.
	 *
	 * @since 4.5.0
	 * @var array
	 */
	...
	/**
	 * Retrieves the registered _partials.
	 *
	 * @since 4.5.0
	 *
	...
	/**
	 * Exports data in preview after it has finished rendering so that _partials can be added at runtime.
	 *
	 * @since 4.5.0
	 */
	...
	/**
	 * Registers dynamically-created _partials.
	 *
	 * @since 4.5.0
	 *
	...
	public function add_dynamic_partials( $partial_ids ) {
		...
		foreach ( $partial_ids as $partial_id ) {
			...
			// Skip _partials already created.
			...
			/**
			 * Filters the class used to construct _partials.
			 *
			 * Allow non-statically created _partials to be constructed with custom WP_Customize_Partial subclass.
			 *
			 * @since 4.5.0
			 *
	...
	/**
	 * Checks whether the request is for rendering _partials.
	 *
	 ...
	 * @since 4.5.0
	 *
	 * @return bool Whether the request is for rendering _partials.
	 */
	...
	/**
	 * Handles PHP errors triggered during rendering the _partials.
	 *
	...
	/**
	 * Handles the Ajax request to return the rendered _partials for the requested placements.
	 *
	 * @since 4.5.0
	 */
	public function handle_render_partials_request() {
		...
		$partials = json_decode( wp_unslash( $_POST['_partials'] ), true );
		...
		/**
		 * Fires immediately before _partials are rendered.
		 *
		 ...
		 * @param array                          $partials Placements' context data for the _partials rendered in the request.
		...
		/**
		 * Fires immediately after _partials are rendered.
		 *
		...
		/**
		 * Filters the response from rendering the _partials.
		 *
		 * Plugins may use this filter to inject `$scripts` and `$styles`, which are dependencies
		 * for the _partials being rendered. The response data will be available to the client via
		 * the `render-_partials-response` JS event, so the client can then inject the scripts and
		 ...
		 *     @type array $errors   List of errors triggered during rendering of _partials, if `WP_DEBUG_DISPLAY`
		 *                           is enabled.
		 * }
		 * @param WP_Customize_Selective_Refresh $this     Selective refresh component.
		 * @param array                          $partials Placements' context data for the _partials rendered in the request.
