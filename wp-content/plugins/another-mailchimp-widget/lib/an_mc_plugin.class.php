...
class AN_MC_Plugin {
	...
	public function init_action() {
		...
		add_action( 'widgets_init', create_function( '', 'return register_widget("AN_Widget_MailChimp");' ) );
				...
				<table class="form-table">
					...
					<?php if ( ! empty( $api_key ) && empty( $mcapi->errorMessage ) ) { ?>
						...
						<?php
							...
							<tr valign="top">
								<th scope="row">
									<?php _e( 'Lists and Groups', 'another-mailchimp-widget' ); ?>
								</th>
								<td><?php
									
									if ( !empty($lists) ) {?>
										<p class="description"><?php _e( 'Name - <code>ID</code>', 'another-mailchimp-widget' ); ?></p>
										<ul>
										<?php
										foreach ($lists as $list) {
											echo '<li>' . $list['title'] . ' - <code>' . $list['id'] . '</code></li>';
												...
												foreach ($list['categories'] as $category) {
													...
													if ( !empty($category['interests']) ) {?>
														...
														foreach ($category['interests'] as $interests) {
															echo '<li style="margin-left:1em;">' . $interests['title'] . ' - <code>' . $interests['id'] . '</code></li>';
														} ?>
														</ul></li><?php
													}
												}
